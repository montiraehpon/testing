...
$data = $med->med_viewProduct($med_idd);
$data2 = $med->med_viewImgProduct($med_idd);
$data3 = $med->med_viewProduct($med_idd);
$data4 = $med->med_viewImgProduct($med_idd);
$data5 = $med->med_viewProduct($med_idd);
...
<form action="" method="POST">
  ...
  <table id="detail" width="80%" height="80%" align="center">
    ...
    <tr style="height: 90%"><hr>
      ...
      ?>
        <tr> <input type="hidden" name="id" size="30" value="<?php echo $row['med_id']?>"><input type="hidden" name="sp_id" size="30" value="<?php echo $row['sp_id']?>"><input type="hidden" name="imgpath" size="30" value="<?php echo $row['med_coverpath']?>"><input type="hidden" name="type" size="30" value="Medicine">
          ...
          <td>: <?=$row['med_name']?> <input type="hidden" name="name" size="30" value="<?php echo $row['med_name']?>"></td>
        </tr>
        <tr>
          <td>Product Price</td>
          <td>: RM <?=$row['med_price']?> <input type="hidden" name="price" size="30" value="<?php echo $row['med_price']?>"></td>    
        </tr>
        <tr>
          <td>Quantity</td>
          <td>: <button name='decqty'>-</button>
        <input type='text' size='1' name='quantity' value='<?= $value; ?>'/>
        <button name='incqty'>+</button>&nbsp; &nbsp; <?=$row["med_quantity"]?> Available</td>
        </tr>
        <tr>
          <td>Variation</td>
    ...
    <tr style="border-top: 1px solid black">
      <td><button type="button" name="back" onclick="window.location.href='ViewProduct.php?med_variation=<?php echo $row['med_variation']?>'"> Back </button></td>
        <?php }?>
