...
if(isset($_POST['update'])){
  $med->med_editProduct($sp_id);   
}

?>
<html>
<head>
  <title>Edit Med Product</title>
...
<body bgcolor="#ccd9ff">
  ...
  <table id="inside_part" width="70%" height="70%" align="center">
    <tr> <hr>
      ...
      ?>
      <input type="hidden" name="cur_med_id" value="<?=$row['med_id']?>">
    <tr>
      <td>Product Name: </td>
      <td><input type="text" name="name" size="30" value="<?php echo $row['med_name']?>" required></td>
    ...
    <tr>
      <td>Product Price: </td>
      <td>RM <input type="number" step="0.01" name="price" value="<?php echo $row['med_price']?>" required> </td>    
      <td colspan="2" rowspan="3" align="center"><img src='<?php echo $image_src?>' id="image" width="160" height="160"/></td>
    </tr>
    <tr>
      <td>Quantity: </td>
      <td><input type="number" name="quantity" value="<?php echo $row['med_quantity']?>" required></td>
    </tr>
    <tr>
      <td>Variation: </td>
      <td><select name="med_variation">
          <option value="" disabled selected value>-Select one med variation-</option>
          <option value="Vitamin" <?php if ($row['med_variation'] == "Vitamin" ) echo 'selected' ; ?> >Vitamin</option>
    ...
    <tr>
      ...
      <td><textarea rows="5" cols="40" name="detail" required><?php echo $row['med_detail']?></textarea></td>
