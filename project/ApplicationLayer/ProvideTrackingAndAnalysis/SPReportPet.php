...
$data = $track->pet_rpt($sp_id);
$datacheck = $track->pet_rpt_check($sp_id);
?>
<html>
<head>
  <style>
...
<table id="btn" align="center">
    ...
    <tr >
      <td><h2 style="text-align: center; height: 10px">Pet Report</h2></td>
    </tr>
  </table>
  <table id="scd_detail" width="70%" height="70%" align="center" rules="all">
    ...
    $subtotal=0;
      if($datacheck == 0){
        ...
        $quantity = 0;
      foreach($data as $row){ 
        $pet_qty_sold = $row["sum"];
        $quantity += $row["product_quantity"];
        $pet_sale =(($row["sum"] * $quantity) * $row["product_price"]);
        $subtotal += $pet_sale;

    ?>
      <tr>
        <td align="center"><?=$row["product_name"]?></td>
        <td align="center"><?=$pet_qty_sold?></td>
        <td align="center">RM <?=$pet_sale?></td>
      </tr>
    <?php } ?>
    <tr style="height: 5%">
