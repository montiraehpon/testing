...
$data = $track->med_rpt($sp_id);
$datacheck = $track->med_rpt_check($sp_id);
?>
<html>
<head>
  <style>
...
<table id="btn" align="center">
    ...
    <tr >
      <td><h2 style="text-align: center; height: 10px">Medicine Report</h2></td>
    </tr>
  </table>
  <table id="scd_detail" width="70%" height="70%" align="center" rules="all">
    ...
    $subtotal=0;
      if($datacheck == 0){
        ...
        $quantity = 0;
      foreach($data as $row){ 
        $med_qty_sold = $row["sum"];
        $quantity += $row["product_quantity"];
        $med_sale =(($row["sum"] * $quantity) * $row["product_price"]);
        $subtotal += $med_sale;

    ?>
      <tr>
        <td align="center"><?=$row["product_name"]?></td>
        <td align="center"><?=$med_qty_sold?></td>
        <td align="center">RM <?=$med_sale?></td>
      </tr>
    <?php } ?>
    <tr style="height: 5%">
