...
$data = $food->food_viewProduct($fd_idd);
$data2 = $food->food_viewImgProduct($fd_idd);
$data3 = $food->food_viewProduct($fd_idd);
$data4 = $food->food_viewImgProduct($fd_idd);
$data5 = $food->food_viewProduct($fd_idd);
  ...
  <table id="detail" width="80%" height="80%" align="center" >
    ...
    <tr style="height: 90%"><hr>
             ...
             <div class="row">
              <div class="column">
                ...
                foreach($data2 as $row){
                  $image2 = $row['imgpath'];
                  $image_src2 = "../../images/FoodImages/".$image2;
      ...
      foreach($data5 as $row){ 
        if($row['fd_variation'] == "Biscuits"){
          $show_variation = "Biscuits & Crackers";
        }
        else if($row['fd_variation'] == "Noodles"){
          $show_variation = "Noodles";
        }
        else if($row['fd_variation'] == "Chips"){
          $show_variation = "Potato Chips & Crips";
        }
        else if($row['fd_variation'] == "Fruits"){
          $show_variation = "Dry Fruits & Nuts";
        }
        else if($row['fd_variation'] == "Others"){
          $show_variation = "Others";
        }        

      ?>
        <tr><input type="hidden" name="id" size="30" value="<?php echo $row['fd_id']?>"><input type="hidden" name="sp_id" size="30" value="<?php echo $row['sp_id']?>"><input type="hidden" name="imgpath" size="30" value="<?php echo $row['fd_coverpath']?>"><input type="hidden" name="type" size="30" value="Food">
          <td width="40%">Product Name  </td>
          <td>: <?=$row['fd_name']?> <input type="hidden" name="name" size="30" value="<?php echo $row['fd_name']?>"></td>
        </tr>
        <tr>
          <td>Product Price</td>
          <td>: RM <?=$row['fd_price']?> <input type="hidden" name="price" size="30" value="<?php echo $row['fd_price']?>"></td>    
        </tr>
        <tr>
          <td>Quantity</td>
          <td>: 
        ...
        <button name='incqty'>+ </button>&nbsp; &nbsp; <?=$row["fd_quantity"]?> Available</td>
        </tr>
        <tr>
          <td>Variation</td>
    ...
    <tr style="border-top: 1px solid black">
      <td>
        <button type="button" name="back" onclick="window.location.href='ViewProduct.php?fd_variation=<?php echo $row['fd_variation']?>'"> Back </button></td>
        <?php }?>
