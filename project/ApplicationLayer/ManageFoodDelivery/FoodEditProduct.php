...
$data = $food->food_view($sp_id,$fd_idd);
...
<body bgcolor="#ccd9ff">
  ...
  <table id="inside_part" width="70%" height="70%" align="center">
    <tr> <hr>
      ...
      ?>
      <input type="hidden" name="cur_fd_id" value="<?=$row['fd_id']?>">
    <tr>
      <td>Product Name: </td>
      <td><input type="text" name="name" size="30" value="<?php echo $row['fd_name']?>" required></td>
    ...
    <tr>
      <td>Product Price: </td>
      <td>RM <input type="number" step="0.01" name="price" value="<?php echo $row['fd_price']?>" required> </td>    
      <td colspan="2" rowspan="3" align="center"><img src='<?php echo $image_src?>' id="image" width="160" height="160"/></td>
    </tr>
    <tr>
      <td>Quantity: </td>
      <td><input type="number" name="quantity" value="<?php echo $row['fd_quantity']?>" required></td>
    </tr>
    <tr>
      <td>Variation: </td>
      <td><select name="food_variation">
          <option value="" disabled selected value>-Select one food variation-</option>
          <option value="Biscuits" <?php if ($row['fd_variation'] == "Biscuits" ) echo 'selected' ; ?> >Biscuits & Crackers</option>
          <option value="Noodles" <?php if ($row['fd_variation'] == "Noodles" ) echo 'selected' ; ?> >Noodles</option>
          <option value="Chips" <?php if ($row['fd_variation'] == "Chips" ) echo 'selected' ; ?> >Potato Chips & Crips</option>
          <option value="Fruits" <?php if ($row['fd_variation'] == "Fruits" ) echo 'selected' ; ?> >Dry Fruits & Nuts</option>
          <option value="Others" <?php if ($row['fd_variation'] == "Others" ) echo 'selected' ; ?> >Others</option>
      </select> </td>
    </tr>
    <tr>
      <td>Product Details: </td>
      <td><textarea rows="5" cols="40" name="detail" required><?php echo $row['fd_detail']?></textarea></td>
  ...
  <?php } ?>
    <tr>
      <td colspan="2"><button type="button" name="back" onclick="window.location.href='FoodProduct.php'"> Back </button></td>
